...
The app is designed to run in a modern browser on phones, tablets, and laptops. Live camera tracking needs HTTPS, camera permission, and browser support for MediaPipe/WebGL. If live tracking is unavailable on a device, the app falls back to demo tracking instead of failing silently.
...
If the GitHub Pages action fails after files were deleted and re-uploaded, upload this bundle again as a new commit or rerun the failed `pages build and deployment` workflow. The live site will keep showing the previous 404 page until a fresh deployment succeeds.
